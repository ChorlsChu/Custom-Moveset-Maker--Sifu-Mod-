...
public partial class ExportDialog : Window
{
    ...
    {
        txtChangeCount.Text = $"{_modifiedNodes.Count} change(s) detected:";
        lstChanges.ItemsSource = _modifiedNodes;
    ...
    private async void Confirm_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            ErrorLog.Write("EXPORT", new Exception($"=== EXPORT START: {_modifiedNodes.Count} modified nodes ==="));

            UpdateStep(1, "done");
            SetProgress(25);

            // Step 2: Patch via UAssetAPI
            UpdateStep(2, "active");
            ...
            // Step 2b: Copy supporting files (attack DBs, data tables)
            UpdateStep(2, "active");
            txtCurrentAction.Text = "Copying supporting files...";

            var fileEntries = new List<(string src, string dest)>
            {
                (outUasset, "../../../Sifu/Content/DB/_MainChar/Combos/MainChar_ComboTree.uasset"),
                (Path.ChangeExtension(outUasset, ".uexp"),
                    "../../../Sifu/Content/DB/_MainChar/Combos/MainChar_ComboTree.uexp")
            };

            // Copy the uexp that UAssetAPI wrote
            var outUexp = Path.ChangeExtension(outUasset, ".uexp");

            // Copy supporting files from extracted paks if they exist
            ...
            // Copy all files to temp for pak creation
            ...
            // Cleanup staging files from output directory
            try
            {
            ...
            // Find game install and copy pak + sig
            ...
            ShowComplete(pakFileName, _modifiedNodes.Count, installedTo);
        }
        catch (Exception ex)
        {
